...
RT_PROGRAM void generateRay()
{
    ...
    float3 raydir = d.x*camera.camera_u + d.y*camera.camera_v + camera.lookdir;
    float  raylength = sqrt(raydir.x*raydir.x + raydir.y*raydir.y + raydir.z*raydir.z);
    float lookdirlen = sqrt(camera.lookdir.x*camera.lookdir.x + camera.lookdir.y*camera.lookdir.y + camera.lookdir.z*camera.lookdir.z);
    float realdepth = (radiancePrd.gt_depth/raylength)*lookdirlen;
    rawOutputDepthBuffer[launchIndex] = realdepth;
